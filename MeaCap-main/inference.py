...
from utils.ifcap_ef_integration import retrieve_concepts_with_ef, retrieve_concepts_with_ef_hybrid
...
if __name__ == "__main__":
    ...
    for batch_idx, (batch_image_embeds, batch_name_list, batch_img_list) in enumerate(train_loader):
        ...
        if args.use_memory:
            ...
            # Extract concepts using IFCap EF module
            if args.use_ef_module:
                if args.ef_hybrid:
                    # Hybrid mode: combine EF and Parser
                    logger.logger.info("Using IFCap EF module (Hybrid mode: EF + Parser)")
                    masked_sentences = retrieve_concepts_with_ef_hybrid(
                        ...
                        ef_fixed_threshold=args.ef_fixed_threshold,
                        use_ef=True,
                    ...
                    # EF module only
                    logger.logger.info("Using IFCap EF module (EF only)")
                    ...
                    logger.logger.info(f"EF extracted concepts: {masked_sentences}")
            else:
                # Use original Parser method
